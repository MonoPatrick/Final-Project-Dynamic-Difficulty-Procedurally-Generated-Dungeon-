...
public class NPC_States : MonoBehaviour
{
    ...
    public void NPCAttack(float damage)
    {
        ...
        if (player.Length > 0)
        {
            PlayerHealth playerHealth = player[0].GetComponent<PlayerHealth>();
    ...
    public void NPCEngage(float damage)
    {
        ...
        if (enemyController.currentNode == null || enemyController.player == null || AStarManager.instance == null)
            return;
    ...
    public void NPCEvade()
    {
        ...
        {
            enemyController.path = AStarManager.instance.GeneratePath(
                enemyController.currentNode,
                AStarManager.instance.FindFurthestNode(enemyController.player.transform.position)
            );
        }
